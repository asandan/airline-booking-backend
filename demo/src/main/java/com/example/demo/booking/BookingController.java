...
public class BookingController {
  ...
  @GetMapping(path = "{id}")
  public ResponseEntity<Booking> findOne(@PathVariable String id)
    throws Exception {
    try {
  ...
  public ResponseEntity<List<Booking>> getBookingByUser(
    @PathVariable String userId
  ) {
    try {
      return ResponseEntity.ok(
        this.bookingService.getBookingByUser(Long.valueOf(userId))
      );
    } catch (Exception e) {
      return ResponseEntity.badRequest().body(null);
    }
